(function () {
  ...
  const SoundFont2SynthMessageType = {
  ...
  */
  class WasmSoundFontSynth {
      ...
      */
      program_select(chan, bank_num, preset_num) {
          wasm.wasmsoundfontsynth_program_select(this.ptr, chan, bank_num, preset_num);
      }
      /**
      ...
      */
      note_on(channel, key, vel, delay_frame) {
          wasm.wasmsoundfontsynth_note_on(this.ptr, channel, key, vel, !isLikeNone(delay_frame), isLikeNone(delay_frame) ? 0 : delay_frame);
      }
      /**
      * @param {number} channel
      * @param {number} key
      ...
      */
      note_off(channel, key, delay_frame) {
          wasm.wasmsoundfontsynth_note_off(this.ptr, channel, key, !isLikeNone(delay_frame), isLikeNone(delay_frame) ? 0 : delay_frame);
      }
      /**
  ...
  async function init(input) {
      if (typeof input === 'undefined') {
          input = new URL('sf2_synth_audio_worklet_wasm_bg.wasm', (document.currentScript && document.currentScript.src || new URL('processor.js', document.baseURI).href));
      }
  ...
  class SoundFont2SynthProcessor extends AudioWorkletProcessor {
      ...
      onmessage(event) {
          ...
          if (data.type === SoundFont2SynthMessageType.SendWasmModule) {
              ...
              this.synth = WasmSoundFontSynth.new(new Uint8Array(this.sf2Bytes), data.sampleRate);
          }
      ...
      noteOn(channel, key, vel, delayTime) {
          if (!this.synth)
              return;
          this.synth.note_on(channel, key, vel, delayTime);
      }
      noteOff(channel, key, delayTime) {
          if (!this.synth)
              return;
